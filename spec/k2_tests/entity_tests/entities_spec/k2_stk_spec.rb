RSpec.describe K2Stk do
  ...
  context '#receive_mpesa_payments' do
    it 'validates input correctly' do
      expect { @k2stk.validate_input(@mpesa_payments, %w[payment_channel till_identifier subscriber amount metadata _links]) }.not_to raise_error
    end

    it 'should add pay recipient request' do
      test_response = SpecStubRequest.mock_stub_request('post', K2Config.path_variable('incoming_payments'), @mpesa_payments,200,
  ...
  context '#query_status' do
    ...
    #  mock_stub_request('get', 'api/v1/incoming_payments', request_body,200, return_response)
    #
