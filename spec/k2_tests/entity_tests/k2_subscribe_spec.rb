...
RSpec.describe K2Subscribe do
  before(:all) do
    @k2subscriber = K2Subscribe.new('webhook_secret')
    @k2sub_not_exist = K2Subscribe.new('webhook_secret')
  ...
  context '#initialize' do
    it 'should initialize with webhook_secret' do
      K2Subscribe.new('webhook_secret')
    end

    it 'should raise an error when there is an empty webhook_secret' do
  ...
  context '#webhook_subscribe' do
    it 'raises error if event type does not match' do
      expect { @k2sub_not_exist.webhook_subscribe('event_type') }.to raise_error ArgumentError
    end

    it 'raises error if event type is empty' do
      expect { @k2subscriber.webhook_subscribe('') }.to raise_error ArgumentError
    end
    ...
    it 'should send webhook subscription for buy goods received' do
      ...
      expect { @k2subscriber.webhook_subscribe('buygoods_transaction_received') }.not_to raise_error
    end

    it 'should send webhook subscription for buy goods reversed' do
      # webhook_subscribe buy goods reversed stub method
      subscription_stub_request('buygoods_transaction_reversed', 'buygoods-transaction-reversed')

      expect { @k2subscriber.webhook_subscribe('buygoods_transaction_reversed') }.not_to raise_error
    end
    ...
    it 'should send webhook subscription for settlement' do
      ...
      expect { @k2subscriber.webhook_subscribe('settlement_transfer_completed') }.not_to raise_error
    end
    ...
    it 'should send webhook subscription for external till to till (b2b)' do
      ...
      expect { @k2subscriber.webhook_subscribe('external_till_to_till') }.not_to raise_error
    end
    ...
    it 'should send webhook subscription for merchant to merchant transaction' do
      ...
      expect { @k2subscriber.webhook_subscribe('k2_merchant_to_merchant') }.not_to raise_error
    end
  end
end
